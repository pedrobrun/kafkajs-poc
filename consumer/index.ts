...
const startConsumer = async (kafka: Kafka) => {
  ...
  const consumer = kafka.consumer({ groupId: args.group })
  ...
  await consumer.run({
    eachMessage: async ({ topic, partition, message }) => {
      console.log(`Consuming message from topic ${topic} and partition ${partition}`)
